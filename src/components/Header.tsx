    ...
    <header className="bg-white dark:bg-gray-900 shadow-md fixed top-0 w-full z-50">
      <div className="container mx-auto px-4 md:px-6 py-4">
        <div className="flex items-center justify-between">
          {/* Logo - Always visible */}
          <Link href="/" className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white flex-shrink-0" onClick={closeMenu}>
            GeoMart
          </Link>

          <div className="flex items-center gap-2 md:gap-4">
            {/* Always visible elements group */}
            <div className="flex items-center gap-2 md:gap-4">
              {/* Location Selector */}
              <select
      ...
      {isMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-black bg-opacity-50" onClick={closeMenu}>
          <div
            className="fixed right-0 top-0 h-full w-64 bg-white dark:bg-gray-900 p-6 shadow-lg transform translate-x-0 transition-transform duration-200 ease-in-out"
              ...
              {!isLoading && (
                isAuthenticated && user ? (
                  <>
                    ...
                    <Link
                      href="/profile"
                      className="text-gray-900 dark:text-white hover:text-indigo-600 transition"
                      onClick={closeMenu}
                    >
                      Profile
                    </Link>
